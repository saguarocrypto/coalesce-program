...
/// Configure blacklist behavior: fail-open (default) or fail-closed.
...
pub fn process(program_id: &Address, accounts: &[AccountView], data: &[u8]) -> ProgramResult {
    ...
    // Parse instruction data: mode (1 byte, 0 = fail-open, 1 = fail-closed)
    if data.is_empty() {
        return Err(ProgramError::InvalidInstructionData);
    }
    let fail_closed = data[0] != 0;
    ...
    config.set_blacklist_mode(fail_closed);

    log!("evt:set_blacklist_mode fail_closed={}", fail_closed);
